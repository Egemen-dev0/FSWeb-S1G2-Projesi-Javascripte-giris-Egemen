...
function oyun(oyuncu, bilgisayar) {
  if (oyuncu == "Taş" && bilgisayar == "Makas" || oyuncu == "Makas" && bilgisayar == "Kağıt" || oyuncu == "Kağıt" && bilgisayar == "Taş") {
    return "Kazandın!"
  } else if (oyuncu == "Taş" && bilgisayar == "Kağıt" || oyuncu == "Makas" && bilgisayar == "Taş" || oyuncu == "Kağıt" && bilgisayar == "Makas") {
    return "Kaybettin!"
...
console.log(oyun("Taş", bilgisayarinSecimii()));
...
function feetDonusturucu(santim) {
  let sonuc = santim / 30.48;
  return sonuc;
}
...
function sesliHarfSayaci(kelime){
  let sayim = 0;  
  const sesliHarfler = ["a", "e", "o","ü", "u", "i", "ı"];
    for (let letter of kelime.toLowerCase()){ 
    if (sesliHarfler.includes(letter)){
        sayim++;
    }
  }
  return sayim;
